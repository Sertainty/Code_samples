/*! 
    ...
    \file     sample_workflow_sm.cpp

    \brief    Sertainty SmartMessage sample program using native C++ language interface.
               Uses XML to create ID.  New ID is used as a single sign-on object. SmartMessage created linking ID.

    \details  A UXP SmartMessage requires authentication of the prospective user.  Access
...
{
  bool licensed;
  std::string outfile("$(SAMPLE)/SampleData/sample_sm.uxp");
  uxl::bytearray idbuf, outbuf, buf;
  int status = StatusChallenged;
  uxp::scFile *sm;

  std::cout << std::endl << "Sample SmartMessage session using C++" << std::endl << std::endl;
  ...
  try
  {
    ...
    std::cout << "Credentials necessary to initiate SSO session:" << std::endl;
    ...
    std::cout << std::endl << "Encoding 'This is a test buffer' with minimal size flag set" << std::endl << std::endl;
    ...
    std::cout << std::endl << "Decoded message: " << buf.data() << std::endl;
