...
void sqlError(uxpSqlQueryHandle e_handle)
{
  if (uxpsys_hasError(uxpsql_errorStatus(e_handle)))
  {
    const char *errMsg = uxpsys_getErrorMessage(uxpsql_errorStatus(e_handle));
    printf("SQL error: %s\n", errMsg);
    exit(1);
  }
}
