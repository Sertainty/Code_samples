/*! \copyright Sertainty Corporation, 2018.  All Rights Reserved.
    ...
    \brief    Sample program using native C++ language interface.  Uses KCL to create ID.
...
int main(int argc, char *argv[])
{
  ...
  uxl::bytearray outbuf, kclfile("$(SAMPLE)/SampleData/sample.kcl");
  uxp::openVFHandle *handle;
  uxl::bytearray buffer;
  int status;
  bool done;

  std::cout << std::endl << std::endl << "Sample C++ UXP Application using KCL" << std::endl << std::endl;

  /* Set up log file and initialize library.  Must do this before any active calls. */

  try
  {
    uxp::sys::setLogFile("SampleC++", "Sample C++ 2.0.0");
